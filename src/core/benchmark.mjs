...
export const BENCHMARK_CASES = [
  ...
  {
    id: "project-specific-yaml-log-bug-template",
    category: "issue",
    name: "Project-specific YAML and logs bug reports count as reproducer evidence",
    input: projectSpecificYamlLogBugIssue(),
    expect: { status: "ready-for-maintainer", labels: ["ready-for-maintainer"], absentLabels: ["needs-reproducer", "needs-expected-actual", "needs-technical-analysis"], repoContext: true }
  },
  {
    id: "numeric-version-root-cause-crash",
    category: "issue",
    name: "Numeric product versions and root-cause crash analysis count as issue evidence",
    input: numericVersionRootCauseCrashIssue(),
    expect: { status: "ready-for-maintainer", labels: ["ready-for-maintainer"], absentLabels: ["needs-environment", "needs-expected-actual"], repoContext: true }
  },
  {
    id: "placeholder-secret-references-not-leaks",
    category: "issue",
    name: "Project config placeholders are not treated as leaked credentials",
    input: placeholderSecretReferenceIssue(),
    expect: { status: "needs-repair", labels: ["needs-technical-analysis"], absentLabels: ["secrets-risk", "low-review-value"], repoContext: true }
  },
  {
    id: "stale-label-prevents-review-now",
    category: "issue",
    name: "Stale maintainer label keeps complete issues out of review-now",
    input: staleMaintainerLabelIssue(),
...
function projectSpecificYamlLogBugIssue() {
  return {
    kind: "issue",
...
function numericVersionRootCauseCrashIssue() {
  return {
    kind: "issue",
...
function placeholderSecretReferenceIssue() {
  return {
    kind: "issue",
...
function staleMaintainerLabelIssue() {
  return {
    kind: "issue",
