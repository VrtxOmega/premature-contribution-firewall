...
export function loadConfig(cwd = process.cwd(), env = process.env) {
  ...
  return {
    ...
    githubSearchDelayMs: clampNumber(env.PCF_GITHUB_SEARCH_DELAY_MS, 3_000, 0, 30_000),
    githubRateLimitRetries: clampNumber(env.PCF_GITHUB_RATE_LIMIT_RETRIES, 2, 0, 5),
    githubRateLimitFallbackMs: clampNumber(env.PCF_GITHUB_RATE_LIMIT_FALLBACK_MS, 60_000, 0, 10 * 60_000),
    githubRateLimitMaxWaitMs: clampNumber(env.PCF_GITHUB_RATE_LIMIT_MAX_WAIT_MS, 5 * 60_000, 0, 30 * 60_000),
    githubRequestTimeoutMs: clampNumber(env.PCF_GITHUB_REQUEST_TIMEOUT_MS, 20_000, 0, 5 * 60_000),
