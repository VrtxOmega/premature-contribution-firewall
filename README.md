...
- The live public record now contains **23 direct external pull-request merges**, including **17 PCF-era direct merges**. The latest addition is [ClickHouse #118352](https://github.com/ClickHouse/ClickHouse/pull/118352), merged September 24 after project-member review and approval. Maintainer-authored Hermes salvage and open credit remain separate from both counts.
- [Hermes Agent #69696](https://github.com/NousResearch/hermes-agent/pull/69696) is an open, maintainer-authored broader solution that explicitly subsumes and credits my still-open [#43834](https://github.com/NousResearch/hermes-agent/pull/43834) for independently mapping the desktop login-shell `PATH` bug family. After another upstream startup refactor, Teknium refreshed the broader solution on August 6, preserved login-shell `PATH` resolution before backend selection and spawn, and reported clean TypeScript plus 12/12 shell-path tests; required CI is green. Neither PR is counted as a merge. The event demonstrates the difference between an aging point patch and a diagnosis that survives into a better system-wide design.
- [Karakeep #2864](https://github.com/karakeep-app/karakeep/pull/2864) merged on July 26, 2026 and shipped in [Karakeep 0.33.1](https://github.com/karakeep-app/karakeep/releases/tag/v0.33.1), whose release notes credit `VrtxOmega` for both the screenshot-scrolling fix and the earlier IPv6 browser-host fix in [#2863](https://github.com/karakeep-app/karakeep/pull/2863). The one-line, component-local overflow correction was validated by the project's format, lint, typecheck, test, diff-check, and pre-commit gates. The long review queue did not broaden the patch or produce a requested technical rework.
...
- [freedesktop-rs/nmrs #521](https://github.com/freedesktop-rs/nmrs/pull/521) merged on August 4, 2026 after the maintainer compared two approaches, selected the contributor's original `InvalidInput` error-return design for the current release, implemented the final revision on the contributor branch, and merged the VrtxOmega-authored PR. The maintainer—not VrtxOmega—authored the final implementation commit.
- [github/awesome-copilot #2476](https://github.com/github/awesome-copilot/pull/2476) merged on August 5, 2026 after human approval and required checks. It adds the non-executing `verify-agent-action` skill and generated install index to GitHub's public community Copilot collection. This is a direct external integration, not GitHub certification or endorsement of PCF or VERITAS.
- [Tokio #8376](https://github.com/tokio-rs/tokio/pull/8376) remains open but received a human `APPROVED` review on September 25; GitHub currently reports it cleanly mergeable. It is **not** counted as a merge until the repository records one.
- The September 25 refresh also records newly closed calibration lanes for .NET root-cause invalidation, an OpenSSL duplicate discovered after branch/repro repair, a catalogue product-shape mismatch, and a ClickHouse backport-routing mistake. Those misses produced new fail-closed gates rather than being hidden from the record.
- [Agent Security Harness #622](https://github.com/msaleme/red-team-blue-team-agent-fabric/issues/622) is a different evidence class: the exact v4.25.0 release reproduced its stated three-pole claim but exposed four additional contentless-response shapes; the external owner independently reproduced one false-assurance row, upstream converted the findings into permanent guards, shipped v4.26.0, and the exact release then passed an independent nine-pole retest. Direct merge totals remain unchanged by this event.
- [AgentTrust governance catalog #48](https://github.com/agentrust-io/awesome-ai-governance/pull/48) closed without merge because the same-day Trust Lab repository did not yet show independent adoption. The curator explicitly called that a timing judgment rather than a verdict on the work and invited a fresh submission after real users exist. PCF records the decline instead of converting it into a positive signal.
